...
4. **Bot (prvýkrát):** `cd scripts/btcpay-config-bot && npm install && sudo npx playwright install-deps chromium && npx playwright install chromium`
...
npm install
# Systémové knižnice pre Chromium (libnspr4, libnss3, …) – potrebné na serveri
sudo npx playwright install-deps chromium
npx playwright install chromium   # len Chromium, stačí na headless
```

Ak uvidíš chybu `libnspr4.so: cannot open shared object file`, chýbajú systémové závislosti – spusti `sudo npx playwright install-deps chromium`.
...
docker exec -it satflux_php_prod php artisan tinker --execute="\$u = \App\Models\User::where('email','bot@satflux.io')->first(); if (\$u) { echo \$u->createToken('btcpay-config-bot')->plainTextToken; } else { echo 'User not found'; }"
```
...
```bash
cd /home/peterhorvath/apps/bitcoin/D21Panel/scripts/btcpay-config-bot
PANEL_BOT_TOKEN="1|Vy3byhHpbIGtgPLe6yo8HVHERfnGljYJolqwhvxn534cfc23" node poll.js --once
```
