  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        'E2EE sync relay: encrypted payloads for local-first invoicing sync between your devices. SatFlux operates a primary relay at relay.satflux.io (when enabled); a secondary public Evolu relay may be used for resilience. SatFlux does not receive plaintext invoicing content from relays; relay operators process encrypted blobs only.',
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        'E2EE sync relay: šifrované payloady pre synchronizáciu local-first fakturácie medzi zariadeniami. SatFlux prevádzkuje primárny relay na relay.satflux.io (ak je zapnutý); pre odolnosť môže byť použitý aj verejný Evolu relay. SatFlux od relay operátorov neprijíma čitateľný obsah faktúr v plaintexte; relay spracúva len šifrované bloby.',
    ...
    if (index === 4) {
      return {
        ...
        paragraphs: [
          ...
          'Relay de sincronización E2EE: cargas cifradas para sincronizar la facturación local-first entre sus dispositivos. SatFlux opera un relay primario en relay.satflux.io (cuando está activo); puede usarse un relay público de Evolu como respaldo. SatFlux no recibe contenido de facturación en texto claro desde los relays; los operadores procesan solo blobs cifrados.',
