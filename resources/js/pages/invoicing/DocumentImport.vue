  ...
  <InvoicingPageShell content-class="pb-10">
    ...
    <nav class="flex flex-wrap gap-2 mb-8 text-sm">
      <span
        ...
        :class="step === idx + 1 ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-gray-50 text-gray-600 border-gray-200'"
      >
        {{ label }}
      </span>
    </nav>
    ...
    <section v-if="step === 1" class="invoicing-card-pad space-y-6 max-w-4xl">
      <h2 class="text-lg font-semibold text-gray-800">{{ t('invoicing.import_step1_heading') }}</h2>
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        ...
        >
          <span class="text-2xl font-bold text-gray-400">{{ sourceIcon(src.id) }}</span>
          <span class="text-sm font-medium text-gray-800">{{ t(src.labelKey) }}</span>
          <span v-if="!src.enabled" class="text-xs text-gray-500">{{ t('invoicing.import_coming_soon') }}</span>
        </button>
      </div>
      <div class="flex justify-end">
        <button type="button" class="invoicing-btn-primary" :disabled="!selectedSource" @click="step = 2">
          {{ t('common.continue') }}
        </button>
      </div>
    </section>
    ...
    <section v-else-if="step === 2" class="space-y-6 max-w-3xl">
      <div class="invoicing-card-pad space-y-4">
        ...
        >
          ...
          <p v-if="selectedFile" class="mt-3 text-sm text-gray-600">{{ selectedFile.name }}</p>
        </div>

        <label class="flex items-center gap-2 text-sm text-gray-700">
          <input v-model="oneRowPerInvoice" type="checkbox" class="rounded border-gray-300" />
          {{ t('invoicing.import_one_row_per_invoice') }}
        </label>

        <label class="flex items-center gap-2 text-sm text-gray-700">
          <input v-model="createContacts" type="checkbox" class="rounded border-gray-300" />
          {{ t('invoicing.import_create_contacts') }}
        </label>
        <p class="text-xs text-gray-500 -mt-2">{{ t('invoicing.import_create_contacts_hint') }}</p>

        <div class="grid sm:grid-cols-2 gap-4">
          <div>
            <label class="invoicing-sf-label">{{ t('invoicing.import_line_name') }}</label>
            <input v-model="lineName" type="text" class="invoicing-sf-input" />
          </div>
          <div>
            <label class="invoicing-sf-label">{{ t('invoicing.import_line_description') }}</label>
            <input v-model="lineDescription" type="text" class="invoicing-sf-input" />
          </div>
        </div>
      </div>

      <div v-if="headers.length" class="invoicing-card-pad space-y-4">
        <h3 class="font-semibold text-gray-800">{{ t('invoicing.import_mapping_title') }}</h3>
        <p class="text-sm text-gray-600">{{ t('invoicing.import_mapping_hint') }}</p>
        ...
        <p class="text-xs text-gray-500">{{ t('invoicing.import_paid_hint') }}</p>
      </div>

      <p v-if="error" class="text-sm text-red-600">{{ error }}</p>

      <div class="flex flex-wrap gap-3">
        <button type="button" class="invoicing-btn-secondary" @click="step = 1">{{ t('common.back') }}</button>
        <button
          type="button"
    ...
    <section v-else class="space-y-6 max-w-4xl">
      <div class="invoicing-card-pad space-y-4">
        <h2 class="text-lg font-semibold text-gray-800">{{ t('invoicing.import_step3_heading') }}</h2>
        <p class="text-sm text-gray-600">
          ...
          <table class="w-full text-sm text-left">
            <thead class="bg-gray-50 text-xs uppercase text-gray-600">
              <tr>
                ...
                <th class="px-3 py-2 text-right">{{ t('invoicing.col_total') }}</th>
                <th class="px-3 py-2">{{ t('invoicing.import_preview_status') }}</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="row in previewRows" :key="row.row" class="border-t border-gray-100">
                <td class="px-3 py-2">{{ row.invoice_number || '—' }}</td>
                <td class="px-3 py-2">{{ row.client_name || '—' }}</td>
                <td class="px-3 py-2 whitespace-nowrap">
                  <template v-if="row.issue_date">{{ row.issue_date }} → {{ row.due_date }}</template>
                  <span v-else class="text-red-600">{{ row.error }}</span>
                </td>
                <td class="px-3 py-2 text-right">
                  <template v-if="row.amount != null">{{ row.amount }} {{ row.currency }}</template>
                </td>
                <td class="px-3 py-2 text-xs">
                  <span v-if="row.paid" class="text-green-700">{{ t('invoicing.filter_paid') }}</span>
                  <span v-else-if="!row.error" class="text-gray-600">{{ t('invoicing.filter_unpaid') }}</span>
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div v-if="importResult" class="text-sm space-y-3 rounded-lg border border-gray-200 bg-gray-50 p-4">
        ...
        <div v-if="importResult.errors.length">
          <p class="font-medium text-amber-900 mb-2">
            {{ t('invoicing.import_skipped_heading', { count: importResult.errors.length }) }}
          </p>
      ...
      <div class="flex flex-wrap gap-3">
        <button type="button" class="invoicing-btn-secondary" :disabled="importing" @click="step = 2">
          {{ t('common.back') }}
        </button>
        <button type="button" class="invoicing-btn-primary px-8" :disabled="importing || !selectedFile" @click="runImport">
...
const mappingReady = computed(() =>
  REQUIRED_DOCUMENT_IMPORT_FIELDS.every((field) => mapping[field] !== null && mapping[field] !== undefined)
);
