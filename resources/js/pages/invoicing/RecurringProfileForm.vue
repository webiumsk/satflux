          ...
          <div v-for="(line, idx) in form.lines" :key="`mobile-line-${idx}`" class="invoicing-line-item-card">
            ...
            <div class="grid grid-cols-2 gap-2">
              ...
              <div>
                ...
                <InvoiceLineUnitSelect v-model="line.unit" class="w-full" />
              </div>
            </div>
            <div class="grid grid-cols-2 gap-2">
              <div>
              ...
              <div v-if="company?.vat_payer">
                <label class="text-xs text-gray-500">{{ t('invoicing.vat') }}%</label>
                <input v-model.number="line.tax_rate" type="number" min="0" max="100" class="invoicing-sf-input w-full" />
              </div>
            </div>
            <div class="flex justify-end">
              <div class="text-right">
                <p class="text-xs text-gray-500">{{ t('invoicing.col_line_total_vat') }}</p>
                <p class="font-medium">{{ lineTotal(line).toLocaleString(locale, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</p>
              </div>
            </div>
          </div>
        </div>
