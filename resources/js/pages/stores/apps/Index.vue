      ...
      <div class="flex-1 flex flex-col overflow-hidden bg-gray-900 border-l border-gray-800">
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar">
          <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <!-- Archived Apps Table -->
            <div v-if="showArchived && displayedApps.length > 0" class="bg-gray-800/50 border border-gray-700 rounded-2xl overflow-hidden">
              <table class="min-w-full divide-y divide-gray-700">
                ...
                <tbody class="divide-y divide-gray-700">
                  ...
                  >
                    <td class="px-6 py-4">
                      <Link :href="`/stores/${store.id}/apps/${app.id}`" class="flex items-center gap-2">
            ...
            <div v-else-if="displayedApps && displayedApps.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <Link
                v-for="app in displayedApps"
                :key="app.id"
              ...
              >
                ...
                <div class="flex items-center mb-4">
                  <div class="p-3 rounded-lg bg-gray-700/50 text-gray-300 group-hover:bg-indigo-500/10 group-hover:text-indigo-400 transition-colors">
                     ...
                     <svg v-else-if="app.app_type === 'Tickets'" class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z" /></svg>
            ...
            <div v-else class="text-center py-16">
               ...
               <template v-if="showArchived">
                 <h3 class="text-xl font-medium text-white mb-2">{{ t('apps.no_archived_apps') }}</h3>
                 <p class="text-gray-400 mb-8 max-w-sm mx-auto">{{ t('apps.no_archived_apps_description') }}</p>
                 <Link
               ...
               <template v-else>
                 <h3 class="text-xl font-medium text-white mb-2">{{ t('apps.no_apps_yet') }}</h3>
                 <p class="text-gray-400 mb-8 max-w-sm mx-auto">{{ t('apps.no_apps_description') }}</p>
                 <Link
                  :href="`/stores/${store.id}/apps/create`"
                  class="inline-flex items-center px-6 py-3 border border-transparent rounded-xl shadow-sm text-base font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors"
                 >
