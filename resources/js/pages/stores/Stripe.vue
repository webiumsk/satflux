  ...
  <div v-else class="flex min-h-0 flex-1 overflow-hidden bg-gray-900">
          ...
          >
            <div>
              <h1 class="text-2xl font-bold text-white mb-1">
                {{ t("stores.stripe_settings_title") }}
              </h1>
              <p class="text-sm text-gray-400">
                {{ t("stores.stripe_settings_for") }}
            ...
            <div class="flex items-center gap-3">
              <button
                ...
                :disabled="
                  formReadonly || !settings?.isConfigured || testingConnection
                "
              ...
              >
                ...
                {{
                  testingConnection
                    ? t("common.loading")
                    : t("stores.stripe_test_connection")
                }}
              </button>
              <button
                type="button"
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
          ...
          >
            ...
            <span class="text-sm font-medium">{{
              t("stores.stripe_test_mode_banner")
            }}</span>
          </div>
          ...
          <form @submit.prevent="saveSettings" class="space-y-8">
              ...
              <div class="flex items-center justify-between">
                <div>
                  <h2 class="text-lg font-semibold text-white">
                    {{ t("stores.stripe_enable") }}
                  </h2>
                  <p class="text-sm text-gray-400 mt-1">
                    {{ t("stores.stripe_enable_description") }}
                  </p>
                </div>
                <button
                  type="button"
            ...
            <div class="bg-gray-800/50 rounded-2xl border border-gray-700 p-6">
              <h2 class="text-lg font-semibold text-white mb-4">
                {{ t("stores.stripe_api_config") }}
              </h2>

              <!-- Configured: read-only status + Clear Credentials -->
              <div
                v-if="settings?.isConfigured"
                class="rounded-xl bg-gray-700/30 border border-gray-600 p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"
              >
                <div>
                  <p class="font-medium text-indigo-400">
                    {{ t("stores.stripe_api_keys_configured") }}
                  </p>
                  <p class="mt-1 text-sm text-gray-400 font-mono">
                    {{ t("stores.stripe_publishable_key") }}:
              ...
              <div v-else class="space-y-5">
                <div>
                  ...
                  >
                    {{ t("stores.stripe_publishable_key") }}
                    <span v-if="form.enabled" class="text-red-400">*</span>
                  </label>
                  <input
                    id="publishableKey"
                  ...
                  <p class="mt-1.5 text-xs text-gray-500">
                    {{ t("stores.stripe_publishable_key_help") }}
                    <a
                      href="https://dashboard.stripe.com/apikeys"
                      target="_blank"
                      rel="noopener"
                      class="text-indigo-400 hover:text-indigo-300"
                      >{{ t("stores.stripe_get_from_dashboard") }}</a
                    >
                  </p>
                  <p
                    v-if="fieldErrors.publishableKey"
                    class="mt-1 text-sm text-red-400"
                  >
                    {{ fieldErrors.publishableKey }}
                  </p>
                </div>
                <div>
                  <label
                  ...
                  />
                  <p class="mt-1.5 text-xs text-gray-500">
                    {{ t("stores.stripe_secret_key_help") }}
                    <a
                      href="https://dashboard.stripe.com/apikeys"
                      target="_blank"
                      rel="noopener"
                ...
                <div>
                  ...
                  />
                  <p class="mt-1.5 text-xs text-gray-500">
                    {{ t("stores.stripe_settlement_currency_help") }}
                  </p>
                  <p
                    v-if="fieldErrors.settlementCurrency"
                    class="mt-1 text-sm text-red-400"
                  >
              ...
              <div
                v-if="settings?.isConfigured"
                class="mt-5 pt-5 border-t border-gray-600"
              >
                <label
                  for="settlementCurrencyConfigured"
                  class="block text-sm font-medium text-gray-300 mb-1"
                >
                  {{ t("stores.stripe_settlement_currency") }}
                </label>
            ...
            <div class="bg-gray-800/50 rounded-2xl border border-gray-700 p-6">
              <h2 class="text-lg font-semibold text-white mb-4">
                {{ t("stores.stripe_webhook_config") }}
              </h2>
              <div
                v-if="!settings?.isConfigured"
                class="rounded-xl bg-gray-700/30 border border-gray-600 p-4 text-gray-400 text-sm"
              >
              ...
              <div v-else class="space-y-3">
                ...
                >
                  <div class="flex items-start gap-3">
                    <div
                      class="flex-shrink-0 w-8 h-8 rounded-full bg-indigo-500/20 flex items-center justify-center"
                    >
                      <svg
                    ...
                    <div class="min-w-0 flex-1">
                      <p class="font-medium text-indigo-400">
                        {{ t("stores.stripe_webhook_active") }}
                      </p>
                      <p
                        v-if="webhookStatus.webhookId"
                        class="mt-1 text-gray-300 text-xs"
                      >
                        {{ t("stores.stripe_webhook_id") }}:
                        <span class="font-mono">{{
                          webhookStatus.webhookId
                        }}</span>
                      </p>
                      <p class="mt-1 text-gray-400 text-xs">
                        {{ t("stores.stripe_webhook_signing_configured") }}
                      </p>
                      <p
                        v-if="webhookStatus.webhookUrl"
                        class="mt-2 text-gray-500 font-mono text-xs break-all"
                      >
                        {{ t("stores.stripe_webhook_url") }}:
                ...
                >
                  <p class="text-gray-300">
                    {{ webhookStatus?.message || t("common.loading") }}
                  </p>
                  <p
                    v-if="webhookStatus?.webhookUrl"
                    class="mt-2 text-gray-500 font-mono text-xs break-all"
                ...
                >
                  ...
                  {{
                    registeringWebhook
                      ? t("common.loading")
                      : t("stores.stripe_register_webhook")
                  }}
                </button>
                <!-- Webhook Signing Secret -->
                <div class="pt-4 border-t border-gray-600">
                  <label
                    for="webhookSigningSecret"
                    class="block text-sm font-medium text-gray-300 mb-1"
                  >
                  ...
                  <input
                    ...
                    :placeholder="
                      settings?.webhookSigningSecret
                        ? t('stores.stripe_key_keep_placeholder')
                        : 'whsec_...'
                    "
                    class="block w-full px-4 py-3 rounded-xl border border-gray-600 bg-gray-700/50 text-white placeholder-gray-500 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 font-mono text-sm disabled:opacity-60 disabled:cursor-not-allowed"
                  />
                  <p class="mt-1.5 text-xs text-gray-500">
                    {{ t("stores.stripe_webhook_signing_secret_help") }}
                    <a
                  ...
                  <p
                    v-if="fieldErrors.webhookSigningSecret"
                    class="mt-1 text-sm text-red-400"
                  >
                    {{ fieldErrors.webhookSigningSecret }}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div class="bg-gray-800/50 rounded-2xl border border-gray-700 p-6">
              <h2 class="text-lg font-semibold text-white mb-4">
                {{ t("stores.stripe_advanced_config") }}
              </h2>
              <div>
                <label
                  for="advancedConfig"
                  class="block text-sm font-medium text-gray-300 mb-1"
                  >{{ t("stores.stripe_advanced_config_json") }}</label
                >
                <textarea
                  id="advancedConfig"
                ...
                ></textarea>
                <p class="mt-1.5 text-xs text-gray-500">
                  {{ t("stores.stripe_advanced_config_help") }}
                  <a
                ...
                <p
                  v-if="fieldErrors.advancedConfig"
                  class="mt-1 text-sm text-red-400"
                >
                  {{ fieldErrors.advancedConfig }}
                </p>
              </div>
            </div>
          </form>
    ...
    <UpgradeModal
      ...
      :message="
        t('upgrade_modal.stripe_available_in_pro', {
          default:
            'Stripe is available in Pro. Upgrade to configure card payments for your store.',
        })
      "
      :limits="[]"
    ...
    >
      ...
      >
        ...
        >
        <div
          class="inline-block align-bottom bg-gray-800 rounded-2xl text-left overflow-hidden shadow-xl transform sm:my-8 sm:align-middle sm:max-w-md sm:w-full border border-gray-700"
        >
            ...
            <div class="sm:flex sm:items-start">
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                <h3 class="text-lg font-bold text-white">
                  {{ t("stores.stripe_clear_confirm_title") }}
                </h3>
                <p class="mt-2 text-sm text-gray-400">
                  {{ t("stores.stripe_clear_confirm_message") }}
                </p>
              </div>
            </div>
          </div>
          <div
            class="bg-gray-800/50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-3 border-t border-gray-700"
          >
            <button
              type="button"
            ...
            >
              {{
                deleting
                  ? t("common.loading")
                  : t("stores.stripe_clear_confirm_button")
              }}
            </button>
            <button
              type="button"
...
function buildPayload() {
  ...
  if (form.value.settlementCurrency?.trim())
    p.settlementCurrency = form.value.settlementCurrency.trim();
  if (form.value.webhookSigningSecret?.trim())
    p.webhookSigningSecret = form.value.webhookSigningSecret.trim();
...
async function saveSettings() {
  ...
  try {
    const res = await api.put(
      `/stores/${storeId.value}/stripe/settings`,
      payload,
    );
    ...
    if (err.response?.status === 422 && data?.errors) {
      ...
      for (const e of Array.isArray(data.errors) ? data.errors : []) {
        const path = (e.path || e.field || "").replace(
          /^([A-Z])/,
...
async function registerWebhook() {
  ...
  try {
    const res = await api.post(
      `/stores/${storeId.value}/stripe/webhook/register`,
    );
    setFlash(
      res.data?.message || t("stores.stripe_webhook_registered"),
      "success",
    );
