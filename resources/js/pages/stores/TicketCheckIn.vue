  ...
  <div class="min-h-screen bg-gray-950 text-white">
    ...
    <header class="bg-gray-900 border-b border-gray-800">
      <div class="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <span class="text-lg font-bold text-white tracking-tight">SATFLUX</span>
        </div>
        <span v-if="eventData" class="text-xs text-gray-500 uppercase tracking-wider">{{ t('tickets.check_in') }}</span>
      </div>
    </header>
    ...
    <div class="max-w-2xl mx-auto px-4 py-6 space-y-6">
      ...
      <div v-if="eventData" class="text-center pb-2">
        ...
        <!-- Stats -->
        <div v-if="stats.total > 0" class="mt-4 flex items-center justify-center gap-3">
          <span class="text-sm text-gray-400">{{ stats.checkedIn }} / {{ stats.total }} {{ t('tickets.checked_in_label') }}</span>
          <div class="w-24 h-2 bg-gray-800 rounded-full overflow-hidden">
            <div class="h-full bg-emerald-500 rounded-full transition-all duration-500" :style="{ width: `${(stats.checkedIn / stats.total) * 100}%` }"></div>
          </div>
        </div>
      </div>
      ...
      <template v-else-if="eventData">
            ...
            <div class="p-6">
              <div class="flex items-start gap-4">
                <div :class="[
                  ...
                  <svg v-if="lastResult.success" class="w-8 h-8 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                  <svg v-else class="w-8 h-8 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
                </div>
                <div class="flex-1 min-w-0">
                  <h3 :class="['text-lg font-semibold mb-1', lastResult.success ? 'text-emerald-400' : 'text-red-400']">
                    {{ lastResult.success ? t('tickets.checkin_ok') : t('tickets.checkin_fail') }}
                  </h3>
                  <p class="text-sm text-gray-300">{{ lastResult.message }}</p>
                  <div v-if="lastResult.ticketInfo" class="mt-3 space-y-1 text-sm text-gray-400">
                    <p v-if="lastResult.ticketInfo.name"><span class="text-gray-500">{{ t('tickets.col_name') }}:</span> {{ lastResult.ticketInfo.name }}</p>
                    <p v-if="lastResult.ticketInfo.email"><span class="text-gray-500">{{ t('tickets.col_email') }}:</span> {{ lastResult.ticketInfo.email }}</p>
                    <p v-if="lastResult.ticketInfo.type"><span class="text-gray-500">{{ t('tickets.col_type') }}:</span> {{ lastResult.ticketInfo.type }}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="bg-gray-800 rounded-2xl border border-gray-700 overflow-hidden">
          <div class="p-6">
            <div class="flex items-center justify-between mb-4">
              ...
              >
                {{ scanning ? t('tickets.stop_scanning') : t('tickets.start_scanning') }}
              </button>
            </div>

            <div v-show="scanning" class="relative rounded-xl overflow-hidden bg-black mb-4">
              <div id="qr-reader" class="w-full"></div>
            </div>

            <p v-if="!scanning" class="text-sm text-gray-500 text-center py-4">{{ t('tickets.scan_hint') }}</p>
          </div>
        </div>
          ...
          <div class="p-6">
            ...
            <form @submit.prevent="handleManualCheckIn" class="flex gap-3">
              <input
                ...
                v-model="manualCode"
                type="text"
                class="flex-1 px-4 py-3 bg-gray-900 border border-gray-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all text-lg font-mono tracking-wider"
              ...
              >
                <svg v-if="checkingIn" class="animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
        ...
        <div v-if="checkinHistory.length > 0" class="bg-gray-800 rounded-2xl border border-gray-700 overflow-hidden">
          <div class="p-6">
            <h2 class="text-base font-semibold text-white mb-4">{{ t('tickets.recent_checkins') }}</h2>
            <div class="space-y-2">
              <div
                ...
                :class="[
                  ...
                  entry.success ? 'bg-emerald-500/5 border border-emerald-500/10' : 'bg-red-500/5 border border-red-500/10'
                ]"
              >
                <svg v-if="entry.success" class="w-4 h-4 text-emerald-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                <svg v-else class="w-4 h-4 text-red-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
                <span class="flex-1 text-sm text-gray-300 truncate">
                  <span v-if="entry.ticketInfo?.name" class="text-white font-medium">{{ entry.ticketInfo.name }}</span>
                  <span v-else class="font-mono text-xs">{{ entry.code }}</span>
                </span>
                <span class="text-xs text-gray-500">{{ formatTime(entry.time) }}</span>
              </div>
            </div>
          </div>
        </div>
...
async function loadEvent() {
  ...
  try {
    const response = await api.get(`/public/ticket-checkin/${storeId}/events/${eventId}`);
...
async function performCheckIn(code: string): Promise<CheckInResult> {
  ...
  if (!ticketCode) {
    return { success: false, message: t('tickets.empty_code'), code: ticketCode, time: new Date() };
  }
  ...
  try {
    ...
    const msg = err?.response?.data?.message
      ...
      || (Array.isArray(err?.response?.data) ? err.response.data.map((e: any) => e.message).join(', ') : '')
      || err?.message
...
async function startScanner() {
  ...
  try {
    ...
    setResult({
      success: false,
      message: t('tickets.camera_error') + (err?.message ? `: ${err.message}` : ''),
      code: '',
      time: new Date(),
    });
  }
