  ...
  >
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <template v-else-if="user">
            ...
            <div>
              ...
              <div class="flex flex-wrap items-center gap-3 mt-2">
                ...
                <span class="text-sm text-gray-400">
                  {{ t("admin.user_detail.registered") }}
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div class="bg-gray-800 rounded-xl p-4 border border-gray-700">
            <p class="text-gray-400 text-xs font-medium mb-1">
              {{ t("admin.user_detail.stores_count") }}
            </p>
          ...
          <div class="bg-gray-800 rounded-xl p-4 border border-gray-700">
            <p class="text-gray-400 text-xs font-medium mb-1">
              {{ t("admin.user_detail.pos_terminals") }}
            </p>
            <p class="text-xl font-bold text-white">{{ posTerminalsTotal }}</p>
          </div>
          <div class="bg-gray-800 rounded-xl p-4 border border-gray-700">
            <p class="text-gray-400 text-xs font-medium mb-1">
              {{ t("admin.user_detail.pos_orders_paid") }}
            </p>
            <p class="text-xl font-bold text-white">
              {{ user.pos_orders_paid_total ?? 0 }}
            </p>
          </div>
          <div class="bg-gray-800 rounded-xl p-4 border border-gray-700">
            <p class="text-gray-400 text-xs font-medium mb-1">
              {{ t("admin.user_detail.subscription") }}
            </p>
        ...
        <div
          v-if="stats?.overall"
          class="bg-gray-800 rounded-xl p-6 border border-gray-700 mb-6"
        >
          <h2 class="text-lg font-semibold text-white mb-4">
            {{ t("admin.user_detail.revenue_stats") }}
          </h2>
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              <p class="text-gray-400 text-sm">
                {{ t("admin.user_detail.invoices_all") }}
              </p>
              <p class="text-white font-semibold">
                {{ stats.overall.invoice_count_all_time ?? 0 }}
              </p>
            </div>
            <div>
              <p class="text-gray-400 text-sm">
                {{ t("admin.user_detail.invoices_30d") }}
              </p>
              <p class="text-white font-semibold">
                {{ stats.overall.invoice_count_30d ?? 0 }}
              </p>
            </div>
            <div>
              <p class="text-gray-400 text-sm">
                {{ t("admin.user_detail.amount_all") }}
              </p>
              <p class="text-white font-semibold">
                {{ formatAmount(stats.overall.paid_amount_all_time) }}
                {{ primaryCurrency }}
              </p>
            </div>
            <div>
              <p class="text-gray-400 text-sm">
                {{ t("admin.user_detail.amount_30d") }}
              </p>
              <p class="text-white font-semibold">
                {{ formatAmount(stats.overall.paid_amount_30d) }}
                {{ primaryCurrency }}
              </p>
            </div>
          </div>
        ...
        >
          <h2
            class="text-lg font-semibold text-white px-6 py-4 border-b border-gray-700"
          >
            {{ t("admin.user_detail.stores") }}
          </h2>
          <div
            v-if="!user.stores?.length"
          ...
          <div v-else class="divide-y divide-gray-700">
            ...
            >
              <div>
                ...
                <div
                  class="flex flex-wrap gap-2 mt-1 text-sm text-gray-400 items-center"
                >
                  <WalletTypeIcon
                  ...
                  <span
                    >{{ t("admin.user_detail.pos_terminals") }}:
                    {{ store.pos_terminal_count }}</span
                  >
                  <span
                    v-if="store.wallet_connection_status"
                    class="px-1.5 py-0.5 rounded text-xs"
                    :class="
                      getWalletStatusClass(store.wallet_connection_status)
                    "
                  >
      ...
      >
        ...
        >
          <h2 class="text-2xl font-bold text-white mb-4">
            {{ t("admin.user_detail.delete_user") }}
          </h2>
          ...
          <div class="flex justify-end gap-3">
            ...
            <button
              ...
              class="px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-lg hover:bg-red-500 disabled:opacity-50"
            >
