<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div v-if="userRole === 'admin'" class="mb-10">
      <h2 class="text-lg font-semibold text-white mb-4">{{ t('admin.dashboard.platform_overview') }}</h2>
      <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-6">
        ...
        >
          <p class="text-gray-400 text-xs font-medium mb-1">{{ t('admin.dashboard.users') }}</p>
          <p class="text-2xl font-bold text-white">{{ loading ? '...' : platformStats.users_total }}</p>
          <p v-if="platformStats.users_7d > 0" class="text-xs text-amber-400 mt-1">{{ t('admin.dashboard.new_users_7d') }} {{ platformStats.users_7d }}</p>
        </div>
        <div
          class="bg-gray-800 rounded-xl p-4 border border-gray-700 hover:border-amber-500/50 transition-all cursor-pointer"
          @click="$router.push('/admin/users')"
        >
          <p class="text-gray-400 text-xs font-medium mb-1">{{ t('admin.dashboard.merchants') }}</p>
          <p class="text-2xl font-bold text-white">{{ loading ? '...' : platformStats.merchants_total }}</p>
        </div>
        <div class="bg-gray-800 rounded-xl p-4 border border-gray-700">
          <p class="text-gray-400 text-xs font-medium mb-1">{{ t('admin.dashboard.stores') }}</p>
          <p class="text-2xl font-bold text-white">{{ loading ? '...' : platformStats.stores_total }}</p>
          <p v-if="platformStats.stores_7d > 0" class="text-xs text-amber-400 mt-1">{{ t('admin.dashboard.new_stores_7d') }} {{ platformStats.stores_7d }}</p>
        </div>
        <div class="bg-gray-800 rounded-xl p-4 border border-gray-700">
          <p class="text-gray-400 text-xs font-medium mb-1">{{ t('admin.dashboard.pos_terminals') }}</p>
          <p class="text-2xl font-bold text-white">{{ loading ? '...' : platformStats.pos_terminals_total }}</p>
        </div>
        <div class="bg-gray-800 rounded-xl p-4 border border-gray-700">
          <p class="text-gray-400 text-xs font-medium mb-1">{{ t('admin.dashboard.pos_orders') }}</p>
          <p class="text-2xl font-bold text-white">{{ loading ? '...' : platformStats.pos_orders_paid_total }}</p>
          <p v-if="posRevenue30d > 0" class="text-xs text-emerald-400 mt-1">{{ formatPosAmount(posRevenue30d) }} (30d)</p>
          <p v-else-if="platformStats.pos_orders_paid_30d > 0" class="text-xs text-emerald-400 mt-1">{{ platformStats.pos_orders_paid_30d }} (30d)</p>
        </div>
        <div
        ...
        >
          <p class="text-gray-400 text-xs font-medium mb-1">{{ t('admin.dashboard.wallet_needs_support') }}</p>
          <p class="text-2xl font-bold" :class="platformStats.wallet_connections_needs_support > 0 ? 'text-amber-400' : 'text-white'">
      ...
      <div class="flex flex-wrap gap-4 mb-6 items-center">
        <div class="bg-gray-800/50 rounded-lg px-4 py-2 border border-gray-700">
          <span class="text-gray-400 text-sm">{{ t('admin.dashboard.paid_plans') }}:</span>
          <span class="text-white font-semibold ml-2">{{ loading ? '...' : platformStats.paid_plan_count }}</span>
        </div>
        <div class="flex items-center gap-2">
          <span class="text-gray-400 text-sm">{{ t('admin.dashboard.currency_toggle') }}:</span>
          <div class="flex rounded-lg border border-gray-600 overflow-hidden">
            <button
              type="button"
              :class="['px-3 py-1.5 text-xs font-medium transition-colors', amountCurrency === 'sats' ? 'bg-amber-600 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-700']"
            ...
            <button
              type="button"
              :class="['px-3 py-1.5 text-xs font-medium transition-colors', amountCurrency === 'eur' ? 'bg-amber-600 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-700']"
      ...
      <!-- Trends chart -->
      <div v-if="trendsChartData.length > 0" class="bg-gray-800 rounded-xl p-6 border border-gray-700">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-base font-semibold text-white">{{ t('admin.dashboard.trends_30d') }}</h3>
          <div class="flex rounded-lg border border-gray-600 overflow-hidden">
            <button
              type="button"
              :class="['px-3 py-1.5 text-xs font-medium transition-colors', trendPeriod === '7d' ? 'bg-indigo-600 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-700']"
            ...
            <button
              type="button"
              :class="['px-3 py-1.5 text-xs font-medium transition-colors', trendPeriod === '30d' ? 'bg-indigo-600 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-700']"
        ...
        <div class="relative h-48 w-full">
          <svg class="w-full h-full overflow-visible" viewBox="0 0 100 100" preserveAspectRatio="none" @mouseleave="trendActivePoint = null">
            <defs>
              <linearGradient id="adminUsersGrad" x1="0%" y1="0%" x2="0%" y2="100%">
                <stop offset="0%" stop-color="#6366f1" stop-opacity="0.3" />
                <stop offset="100%" stop-color="#6366f1" stop-opacity="0" />
              </linearGradient>
              <linearGradient id="adminStoresGrad" x1="0%" y1="0%" x2="0%" y2="100%">
            ...
            <g v-for="(d, i) in trendsChartData" :key="d.date">
              ...
              <!-- Users bar -->
              <rect
                :x="(i / trendsChartData.length) * 100 + (100 / trendsChartData.length) * 0.02"
                :y="100 - (d.users / maxTrendValue) * 40"
                :width="(100 / trendsChartData.length) * 0.28"
                :height="Math.max((d.users / maxTrendValue) * 40, d.users > 0 ? 2 : 0)"
                fill="#6366f1"
              ...
              <!-- Stores bar -->
              <rect
                :x="(i / trendsChartData.length) * 100 + (100 / trendsChartData.length) * 0.35"
                :y="100 - (d.stores / maxTrendValue) * 40"
                :width="(100 / trendsChartData.length) * 0.28"
                :height="Math.max((d.stores / maxTrendValue) * 40, d.stores > 0 ? 2 : 0)"
                fill="#10b981"
                :opacity="trendActivePoint === i ? 1 : 0.7"
                rx="2"
                class="transition-all duration-150"
              ...
              <rect
                :x="(i / trendsChartData.length) * 100 + (100 / trendsChartData.length) * 0.68"
                :y="100 - (posAmountForDay(d) / maxTrendValue) * 40"
                :width="(100 / trendsChartData.length) * 0.28"
                :height="Math.max((posAmountForDay(d) / maxTrendValue) * 40, posAmountForDay(d) > 0 ? 2 : 0)"
                fill="#f59e0b"
                :opacity="trendActivePoint === i ? 1 : 0.7"
                rx="2"
                class="transition-all duration-150"
          ...
          >
            ...
            <div class="flex items-center justify-between gap-4">
              ...
              <span class="font-bold text-indigo-400">{{ trendsChartData[trendActivePoint]?.users ?? 0 }}</span>
            </div>
            <div class="flex items-center justify-between gap-4 mt-1">
              ...
              <span class="font-bold text-emerald-400">{{ trendsChartData[trendActivePoint]?.stores ?? 0 }}</span>
            </div>
            <div class="flex items-center justify-between gap-4 mt-1">
              <span class="flex items-center gap-2 text-sm text-gray-300">
              ...
              <span class="font-bold text-amber-400">{{ formatPosAmount(posAmountForDay(trendsChartData[trendActivePoint])) }}</span>
            </div>
          </div>
        </div>
        <div class="flex justify-between mt-2 text-xs text-gray-500">
          <span>{{ formatChartDate(trendsChartData[0]?.date) }}</span>
          <span>{{ formatChartDate(trendsChartData[trendsChartData.length - 1]?.date) }}</span>
        </div>
        <div class="flex justify-between items-center mt-3">
          ...
          >
            <svg v-if="exportingStats" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
      ...
      <div class="bg-gray-800 rounded-xl p-6 border border-gray-700 mt-6">
        <h3 class="text-base font-semibold text-white mb-4">{{ t('admin.dashboard.top_stores_pos') }}</h3>
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-700">
            <thead class="bg-gray-900/50">
              <tr>
                <th class="px-4 py-2 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">#</th>
                <th class="px-4 py-2 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">{{ t('admin.dashboard.stores') }}</th>
                <th class="px-4 py-2 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">{{ t('admin.dashboard.merchants') }}</th>
                <th class="px-4 py-2 text-right text-xs font-medium text-gray-400 uppercase tracking-wider">{{ t('admin.dashboard.pos_orders') }}</th>
                <th class="px-4 py-2 text-right text-xs font-medium text-gray-400 uppercase tracking-wider">{{ amountCurrency === 'sats' ? 'sats' : '€' }}</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-700">
              <tr v-if="platformStats.top_stores_by_pos_orders.length === 0">
                <td colspan="5" class="px-4 py-8 text-center text-gray-400">{{ t('admin.dashboard.top_stores_empty') }}</td>
              </tr>
              <tr
              ...
              >
                ...
                <td class="px-4 py-3 text-sm font-medium text-white">{{ s.store_name }}</td>
                <td class="px-4 py-3 text-sm text-gray-400">{{ s.user_email || '—' }}</td>
                <td class="px-4 py-3 text-sm text-right font-semibold text-emerald-400">{{ s.pos_orders_paid }}</td>
                <td class="px-4 py-3 text-sm text-right font-medium text-amber-400">{{ formatPosAmount(amountCurrency === 'sats' ? (s.pos_amount_sats ?? 0) : (s.pos_amount_eur ?? 0)) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
const platformStats = ref({
  ...
  paid_plan_count: 0,
  trends_7d: [] as { date: string; users: number; stores: number; pos_orders: number; pos_amount_sats?: number; pos_amount_eur?: number }[],
  trends_30d: [] as { date: string; users: number; stores: number; pos_orders: number; pos_amount_sats?: number; pos_amount_eur?: number }[],
  top_stores_by_pos_orders: [] as { store_id: string; store_name: string; user_email: string | null; pos_orders_paid: number; pos_amount_sats?: number; pos_amount_eur?: number }[],
});
...
function posAmountForDay(d: { pos_amount_sats?: number; pos_amount_eur?: number } | undefined): number {
  if (!d) return 0;
  return amountCurrency.value === 'sats' ? (d.pos_amount_sats ?? 0) : (d.pos_amount_eur ?? 0);
}
...
const maxTrendValue = computed(() => {
  ...
  type TrendDay = { users: number; stores: number; pos_amount_sats?: number; pos_amount_eur?: number };
  const max = Math.max(
