<template>
  <div
    v-show="flash.show"
    class="fixed top-4 right-4 z-[9999] flex items-center w-full max-w-xs p-4 space-x-4 text-white rounded-lg shadow-lg dark:text-gray-400 dark:bg-gray-800 transition-all duration-300 transform cursor-default"
    @mouseenter="flash.pauseAutoHide()"
    @mouseleave="flash.resumeAutoHide()"
    :class="[
      bgClass,
...
const bgClass = computed(() => {
  if (flash.type === "warning")
    return "bg-gray-800 border-l-4 border-orange-500";
...
const iconBgClass = computed(() => {
  if (flash.type === "warning")
    return "text-orange-500 bg-orange-100 rounded-lg dark:bg-orange-900/50 dark:text-orange-400";
  return flash.type === "success"
