<template>
  <aside class="w-full lg:w-56 shrink-0 space-y-2.5">
    ...
    <div
      v-if="status === 'paid' && paidAt"
      class="rounded-lg border border-emerald-300 bg-emerald-50 p-3 text-sm"
    >
      <div class="text-emerald-800 font-medium mb-1">
        {{ t("invoicing.expense_status_paid") }}
      </div>
    ...
    <div v-if="status !== 'cancelled'" class="relative pt-1">
      ...
      <div
        v-if="showIssueMenu"
        class="invoicing-dropdown left-0 right-0 min-w-0 w-full"
      >
        <button
          v-if="hasAttachment"
          type="button"
...
const statusBanner = computed(() => {
  if (props.status === "cancelled") {
    return {
      label: t("invoicing.expense_status_cancelled"),
      class: "bg-gray-500",
    };
  }
  if (props.status === "paid") {
    return {
      label: t("invoicing.expense_status_paid"),
      class: "bg-emerald-500",
    };
  }
  if (props.isOverdue) {
    return {
      label: t("invoicing.expense_filter_overdue"),
      class: "bg-red-500",
    };
  }
  if (props.status === "recorded") {
    return {
      label: t("invoicing.expense_filter_unpaid"),
      class: "bg-amber-500",
    };
  }
