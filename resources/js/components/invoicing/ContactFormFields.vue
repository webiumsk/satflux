<template>
  <div class="space-y-8">
    <template v-if="activeTab === 'billing'">
      ...
      <div v-else>
        <label class="invoicing-sf-label">{{
          t("invoicing.contact_name_label")
        }}</label>
        <p
          class="contact-readonly-value text-lg font-semibold text-center py-2"
        >
          {{ form.name || "-" }}
        </p>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="space-y-3">
          <div>
            ...
            />
            <p v-else class="contact-readonly-value whitespace-pre-wrap">
              {{ form.street || "-" }}
            </p>
          </div>
        ...
        <div class="space-y-3">
          ...
          <div v-if="showSkEfakturaFields">
            <label class="invoicing-sf-label">{{
              t("invoicing.contact_peppol_participant_id")
            }}</label>
            <input
              v-if="!readonly"
              ...
              :placeholder="
                t('invoicing.contact_peppol_participant_id_placeholder')
              "
            />
            <p v-else class="contact-readonly-value font-mono text-sm">
              {{ form.peppol_participant_id || "-" }}
            </p>
            <p v-if="!readonly" class="text-xs text-gray-500 mt-1">
              {{ t("invoicing.contact_peppol_participant_id_hint") }}
            </p>
          </div>
          <div>
            <label class="invoicing-sf-label">{{
            ...
            <div v-if="!readonly" class="flex gap-2 items-start">
              ...
              >
                {{
                  viesLoading
                    ? t("invoicing.vies_checking")
                    : t("invoicing.vies_validate")
                }}
              </button>
            </div>
            <p v-else class="contact-readonly-value">
            ...
            <p v-if="!readonly" class="text-xs text-gray-500 mt-1">
              {{ t("invoicing.vat_number_hint") }}
            </p>
            <p
              v-if="!readonly && viesFeedback"
              class="text-xs mt-1"
              :class="viesFeedbackClass"
            >
              {{ viesFeedback }}
            </p>
          </div>
      ...
      <div
        class="grid grid-cols-2 md:grid-cols-4 gap-4 border-t border-gray-100 pt-6"
      >
        <div>
          <label class="invoicing-sf-label">{{
            t("invoicing.bank_account")
          }}</label>
          <input
            v-if="!readonly"
        ...
        <div>
          ...
          <input
            ...
            class="invoicing-sf-input"
          />
          <p v-else class="contact-readonly-value break-all">
            {{ form.iban || "-" }}
          </p>
        </div>
        <div>
      ...
      <div>
        <div class="invoicing-section-divider">
          {{ t("invoicing.contact_persons") }}
        </div>
        <div
        ...
        >
          <div>
            <label v-if="idx === 0" class="invoicing-sf-label">{{
              t("invoicing.contact_person_name")
            }}</label>
            <input
              v-if="!readonly"
              v-model="person.name"
    ...
    <div v-if="activeTab === 'defaults'" class="max-w-xs">
      <label class="invoicing-sf-label">{{
        t("invoicing.default_payment_terms")
      }}</label>
      <input
        v-if="!readonly"
...
const {
  loading: viesLoading,
  message: viesMessage,
  lastResult: viesResult,
  validate: validateVies,
} = useViesValidation();
const viesFeedback = computed(() => {
  if (viesMessage.value === "unavailable")
    return t("invoicing.vies_unavailable");
...
const showUsAddressFields = computed(() => {
  ...
  return (
    ...
    registryCountry.value === "us"
  );
});

const { enabled: efakturaGloballyEnabled, load: loadEfakturaFeature } =
  useEfakturaFeature();
