<template>
  <div class="invoice-preview bg-white text-gray-900 rounded-lg shadow-lg text-sm p-8 min-h-[720px]">
    <div class="grid grid-cols-3 gap-4 mb-3">
      <div class="text-[10px] leading-relaxed text-gray-700">
        <div class="text-[9px] uppercase font-bold text-gray-500 mb-1 tracking-wide">
        ...
        <div v-if="company?.postal_code || company?.city">
          {{ company.postal_code }} {{ company.city }}<span v-if="company?.country">, {{ company.country }}</span>
        </div>
        <div v-if="company?.registration_number">{{ t('invoicing.registration_number') }}: {{ company.registration_number }}</div>
        <div v-if="company?.tax_id">{{ t('invoicing.tax_id_dic') }}: {{ company.tax_id }}</div>
        <div v-if="company?.vat_number">{{ t('invoicing.vat_number_ic_dph') }}: {{ company.vat_number }}</div>
        <div v-if="company?.commercial_register" class="text-gray-500">{{ company.commercial_register }}</div>
      </div>
      ...
      <div class="text-right">
        <div class="text-2xl font-bold text-gray-900 leading-tight">{{ invoiceHeading }}</div>
        <div v-if="form.variable_symbol" class="text-xs text-gray-600 mt-1">
          {{ t('invoicing.variable_symbol') }}: <strong>{{ form.variable_symbol }}</strong>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-2 gap-6 text-xs mb-3">
      <div class="text-gray-700 space-y-0.5">
        <div v-if="company?.bank_name">
          <span class="text-gray-500">{{ t('invoicing.bank_name') }}:</span> {{ company.bank_name }}
        </div>
        <div v-if="company?.iban">
          <span class="text-gray-500">IBAN:</span> {{ formatIban(company.iban) }}
        </div>
        <div v-if="company?.bank_account">
          <span class="text-gray-500">{{ t('invoicing.bank_account') }}:</span>
          {{ company.bank_account }}<span v-if="company.bank_code"> / {{ company.bank_code }}</span>
        </div>
        <div v-if="form.variable_symbol">
          <span class="text-gray-500">{{ t('invoicing.variable_symbol') }}:</span> {{ form.variable_symbol }}
        </div>
        <div v-if="form.constant_symbol">
          <span class="text-gray-500">{{ t('invoicing.constant_symbol') }}:</span> {{ form.constant_symbol }}
        </div>
      </div>
      <div>
        <div class="text-[9px] uppercase font-bold text-gray-500 mb-1 tracking-wide">
        ...
        <template v-if="selectedContact">
          <strong class="text-sm text-gray-900">{{ selectedContact.name }}</strong>
        ...
        <table class="w-full mt-3 text-xs">
          <tbody>
            <tr>
              <td class="text-right pr-2 text-gray-500">{{ t('invoicing.issue_date') }}:</td>
              <td class="text-right font-semibold">{{ formatDate(form.issue_date) }}</td>
            </tr>
    ...
    <p v-if="form.note_above_lines" class="text-xs text-gray-600 mb-3 whitespace-pre-wrap">{{ form.note_above_lines }}</p>

    <table class="w-full border-collapse text-xs mb-4">
      <thead>
        <tr class="border-b-2 border-gray-800 text-[9px] uppercase text-gray-600">
          <th class="text-left py-2 font-bold">{{ t('invoicing.col_item') }}</th>
          <th class="text-right py-2 font-bold w-16">{{ t('invoicing.col_qty') }}</th>
          <th class="text-center py-2 font-bold w-14">{{ t('invoicing.col_unit') }}</th>
          <th class="text-right py-2 font-bold w-24">{{ t('invoicing.col_unit_price') }}</th>
          <th class="text-right py-2 font-bold w-24">{{ t('invoicing.col_line_total') }}</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="(line, idx) in form.lines" :key="idx" class="border-b border-gray-200">
          <td class="py-2.5 pr-2">
            <div class="font-semibold text-gray-900">{{ line.name || '—' }}</div>
            <div v-if="line.description" class="text-gray-500 text-[10px] mt-0.5">{{ line.description }}</div>
          </td>
          <td class="text-right py-2.5 tabular-nums">{{ line.quantity }}</td>
          <td class="text-center py-2.5">{{ line.unit }}</td>
          <td class="text-right py-2.5 tabular-nums">{{ formatMoney(line.unit_price) }} {{ form.currency }}</td>
          <td class="text-right py-2.5 font-semibold tabular-nums">{{ formatMoney(lineTotal(line)) }} {{ form.currency }}</td>
        </tr>
      </tbody>
    </table>
    ...
    <div class="flex gap-8 mt-2">
      <div v-if="form.note_footer" class="flex-1 text-xs text-gray-600">
        <div class="text-[9px] uppercase font-bold text-gray-500 mb-1">{{ t('invoicing.note_short') }}</div>
        <div class="whitespace-pre-wrap">{{ form.note_footer }}</div>
      </div>
      <div class="w-60 shrink-0 ml-auto space-y-1 text-xs">
        <div v-if="form.discount_percent > 0" class="flex justify-between text-gray-600">
          <span>{{ t('invoicing.discount') }} {{ form.discount_percent }}%</span>
        </div>
        ...
        <template v-if="showTaxSummary && totals.tax_breakdown?.length">
          ...
          >
            ...
            <span>{{ formatMoney(parseFloat(row.tax_amount)) }} {{ form.currency }}</span>
          </div>
        </template>
        <div v-else-if="showTaxSummary" class="flex justify-between text-gray-600">
          <span>{{ isUsCompany ? t('invoicing.sales_tax') : t('invoicing.vat') }}</span>
    ...
    >
      <div>
        <div class="text-[8px] uppercase font-bold text-sky-700 mb-0.5">IBAN</div>
        <div class="font-medium">{{ formatIban(company.iban) }}</div>
      </div>
      <div v-if="form.variable_symbol">
        <div class="text-[8px] uppercase font-bold text-sky-700 mb-0.5">{{ t('invoicing.variable_symbol') }}</div>
        <div class="font-medium">{{ form.variable_symbol }}</div>
      </div>
      <div v-if="form.due_date">
        <div class="text-[8px] uppercase font-bold text-sky-700 mb-0.5">{{ t('invoicing.due_date') }}</div>
        <div class="font-medium">{{ formatDate(form.due_date) }}</div>
      </div>
      <div>
        <div class="text-[8px] uppercase font-bold text-sky-700 mb-0.5">{{ t('invoicing.amount_due_sum') }}</div>
        <div class="text-sm font-bold">{{ formatMoney(amountDue) }} {{ form.currency }}</div>
      </div>
    </div>

    <p v-if="company?.issuer_name || company?.issuer_email" class="text-[10px] text-gray-500 mt-4">
...
function lineTotal(line: InvoiceLineForm) {
  const net = (line.quantity || 0) * (line.unit_price || 0) * (1 - (line.line_discount_percent || 0) / 100);
