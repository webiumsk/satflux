<template>
  <div
    class="expense-table-wrap border-b border-gray-200 overflow-x-auto hidden md:block"
  >
      ...
      >
        <tr>
          <th class="w-12 px-2 py-3 relative text-center">
            ...
            >
              <template v-if="selectionCount > 0">
                <button
                  type="button"
                  class="invoicing-dropdown-item text-gray-600"
                ...
                <button
                  type="button"
                  class="invoicing-dropdown-item"
                  @click="$emit('run-bulk', 'export_xlsx')"
                >
                ...
                <button
                  type="button"
                  class="invoicing-dropdown-item"
                  @click="$emit('run-bulk', 'attachments_zip')"
                >
          ...
          <th class="px-4 py-3 min-w-[180px]">
            {{ t("invoicing.expense_col_name_desc") }}
          </th>
          <th class="px-4 py-3 min-w-[100px]">
            {{ t("invoicing.expense_col_category") }}
          </th>
          <th class="px-4 py-3 min-w-[140px]">
            {{ t("invoicing.expense_col_supplier") }}
          </th>
          <th class="px-4 py-3 min-w-[110px]">
            {{ t("invoicing.expense_col_vs_doc") }}
          </th>
          <th class="px-4 py-3 text-right min-w-[96px]">
            {{ t("invoicing.expense_col_total") }}
          </th>
          <th class="px-4 py-3 text-right min-w-[130px]">
            {{ t("invoicing.expense_col_dates_block") }}
          </th>
      ...
      <tbody>
        <tr
          ...
          :class="
            isSelected(row.id)
              ? 'bg-indigo-50'
              : 'bg-white hover:bg-indigo-50/60 hover:border-b border-gray-200'
          "
        >
          ...
          <td
            class="relative w-4 p-0 align-stretch cursor-pointer"
          ...
          <td
            class="px-4 py-3 align-middle cursor-pointer"
            @click="$emit('open', row.id)"
          >
            <div class="flex items-start gap-2 min-w-0">
              ...
              >
                <IconPaperclip
                  icon-class="w-[15px] h-[15px] -rotate-45"
                  :stroke-width="1.75"
                />
              </span>
              <div class="min-w-0">
                <span
                  class="font-semibold text-indigo-600 group-hover:text-indigo-800 group-hover:underline"
                >
                  {{ displayTitle(row) }}
                </span>
          ...
          <td
            class="px-4 py-3 text-gray-600 align-middle cursor-pointer"
            @click="$emit('open', row.id)"
          >
          ...
          <td
            ...
            @click="$emit('open', row.id)"
          >
            <span class="text-indigo-600 group-hover:underline">{{
              metaFor(row).supplier || "-"
            }}</span>
          </td>

          <td
            class="px-4 py-3 text-gray-700 align-middle text-xs leading-relaxed cursor-pointer"
            @click="$emit('open', row.id)"
          >
          ...
          <td
            class="px-4 py-3 text-right align-middle whitespace-nowrap relative"
          >
            <div
              class="cursor-pointer"
              :class="
                row.is_overdue ? 'text-red-600 font-medium' : 'text-gray-700'
              "
              @click="$emit('open', row.id)"
            >
            ...
            >
              {{
                t("invoicing.expense_overdue_days", {
                  count: overdueDays(row.due_date),
                })
              }}
            </div>
...
function editTo(expenseId: string) {
  return {
    ...
    params: { companyId: props.companyId, expenseId },
  };
}
