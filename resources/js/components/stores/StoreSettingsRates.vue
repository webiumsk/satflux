<template>
  <div class="space-y-6">
    <h2 class="text-lg font-semibold text-white">
      {{ t("stores.settings_tab_rates") }}
    </h2>
    <p class="text-sm text-gray-400 max-w-2xl">
      {{ t("stores.settings_tab_rates_desc") }}
    </p>
    <div>
      <label
        for="preferred_exchange"
        class="block text-sm font-medium text-gray-300 mb-1"
        >{{ t("stores.preferred_price_source") }}</label
      >
      <Select
      ...
      <p class="mt-2 text-xs text-gray-500">
        {{ t("stores.recommended_price_source") }}
      </p>
    </div>
    <div class="space-y-4 pt-4 border-t border-gray-700">
      <div class="flex items-center gap-2 flex-wrap">
        <h3 class="text-base font-semibold text-white">
          {{ t("stores.settings_additional_rates") }}
        </h3>
        <button
      ...
      >
        <div>
          <label
            for="additional_tracked_rates"
            class="block text-sm font-medium text-gray-300 mb-1"
            >{{ t("stores.settings_additional_rates_label") }}</label
          >
          <input
          ...
          <p class="mt-1 text-xs text-gray-500">
            {{ t("stores.settings_additional_rates_desc") }}
          </p>
        </div>
      </div>
    </div>
...
function onAdditionalRatesInput(e: Event) {
  ...
  props.form.additional_tracked_rates = raw
    .split(",")
    .map((s: string) => s.trim())
    .filter(Boolean);
}
