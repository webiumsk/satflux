<template>
  <div class="space-y-6">
    <h2 class="text-lg font-semibold text-white border-b border-gray-700 pb-2">
      {{ t("stores.settings_tab_payment") }}
    </h2>
    <p class="text-sm text-gray-400 max-w-2xl">
      {{ t("stores.settings_tab_payment_desc") }}
    </p>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div>
        <label
          ...
          >{{ t("stores.default_currency") }}</label
        >
        <input
          id="default_currency"
    ...
    <div class="space-y-4 pt-4 border-t border-gray-700">
      <div class="flex flex-wrap gap-x-8 gap-y-4 items-start">
        <label class="flex items-center">
          <input
            v-model="form.anyone_can_create_invoice"
            type="checkbox"
            class="h-5 w-5 text-indigo-600 focus:ring-indigo-500 border-gray-600 bg-gray-800 rounded"
          />
          <span class="ml-2 text-sm text-gray-300">{{
            t("stores.settings_allow_anyone_invoice")
          }}</span>
        </label>
        <div>
          <label class="flex items-center">
            <input
              v-model="form.show_recommended_fee"
              type="checkbox"
              class="h-5 w-5 text-indigo-600 focus:ring-indigo-500 border-gray-600 bg-gray-800 rounded"
            />
            <span class="ml-2 text-sm text-gray-300">{{
              t("stores.settings_show_recommended_fee")
            }}</span>
          </label>
          <p
            v-if="form.show_recommended_fee"
            class="mt-1 ml-7 text-xs text-gray-500"
          >
            {{ t("stores.settings_fee_btc_ltc_only") }}
          </p>
        </div>
      </div>
      <div v-if="form.show_recommended_fee" class="max-w-xs">
        <label
          for="recommended_fee_block_target"
          class="block text-sm font-medium text-gray-300 mb-1"
          >{{ t("stores.settings_recommended_fee_blocks") }}</label
        >
        <input
          ...
          type="number"
          min="1"
          class="appearance-none block w-full px-4 py-2 border border-gray-600 rounded-xl shadow-sm text-white bg-gray-700/50 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors"
        />
      </div>
    </div>
    ...
    <div class="space-y-4 pt-4 border-t border-gray-700">
      <div class="flex items-center gap-2 flex-wrap">
        <h3 class="text-base font-semibold text-white">
          {{ t("stores.settings_payment_options") }}
        </h3>
        <button
      ...
      >
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label
              for="network_fee_mode"
              class="block text-sm font-medium text-gray-300 mb-1"
              >{{ t("stores.settings_add_network_fee") }}</label
            >
            <Select
              id="network_fee_mode"
          ...
          <div>
            <label
              for="invoice_expiration"
              class="block text-sm font-medium text-gray-300 mb-1"
              >{{ t("stores.settings_invoice_expires") }}</label
            >
            <input
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label
              for="payment_tolerance"
              class="block text-sm font-medium text-gray-300 mb-1"
              >{{ t("stores.settings_consider_paid_less") }}</label
            >
            <input
          ...
          <div>
            <label
              for="refund_bolt11_expiration"
              class="block text-sm font-medium text-gray-300 mb-1"
              >{{ t("stores.settings_bolt11_refund_days") }}</label
            >
            <input
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label
              for="monitoring_expiration"
              class="block text-sm font-medium text-gray-300 mb-1"
              >{{ t("stores.settings_payment_invalid_after") }}</label
            >
            <input
        ...
        <div>
          <label
            for="speed_policy"
            class="block text-sm font-medium text-gray-300 mb-1"
            >{{ t("stores.settings_invoice_settled_when") }}</label
          >
          <Select
            id="speed_policy"
        ...
        <div>
          <label
            for="lightning_description_template"
            class="block text-sm font-medium text-gray-300 mb-1"
            >{{ t("stores.settings_lightning_description_template") }}</label
          >
          <input
            id="lightning_description_template"
            v-model="form.lightning_description_template"
            type="text"
            class="appearance-none block w-full px-4 py-2 border border-gray-600 rounded-xl shadow-sm placeholder-gray-500 text-white bg-gray-700/50 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors"
          />
        </div>
      </div>
    </div>
