  ...
  <div class="space-y-4">
    <div>
      <label class="block text-sm font-medium text-gray-300 mb-1"
        >{{ t("raffles.field_ticket_currency") }} *</label
      >
      <select
    ...
    <div>
      ...
      <input
        ...
        @input="onPriceInput"
      />
      <p v-if="isSats" class="mt-1 text-xs text-gray-500">
        {{ t("raffles.field_ticket_price_sats_hint") }}
      </p>
    </div>
  </div>
</template>
...
const priceLabel = computed(() =>
  isSats.value
    ? t("raffles.field_ticket_price_sats")
    : t("raffles.field_ticket_price"),
);
...
function onCurrencyChange(event: Event) {
  const next = normalizeRaffleCurrency(
...
defineExpose({
  toPayload(): RafflePricingForm {
    return {
      ticketCurrency: props.ticketCurrency,
