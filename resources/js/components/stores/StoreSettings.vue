<template>
  ...
  <div v-else-if="settings" class="space-y-6">
    ...
    <div class="bg-gray-800 shadow-xl rounded-2xl border border-gray-700">
      <div class="px-6 py-5 border-b border-gray-700 bg-gray-800/50">
        ...
        <p class="text-sm text-gray-400 mt-1">
          {{ t("stores.manage_store_config") }}
        </p>
      </div>

      <!-- Tabs -->
      <nav
        class="flex border-b border-gray-700 overflow-x-auto"
      ...
      <div
        v-show="
          ['settings', 'payment', 'rates', 'checkout', 'lightning'].includes(
            activeSettingsTab,
          )
        "
        class="px-6 py-8"
      >
        <form @submit.prevent="handleSettingsSubmit" class="space-y-8">
          ...
          <StoreSettingsCheckout
            ...
            @show-upgrade="showUpgradeModal = true"
            :payment-method-criteria-type-options="
              paymentMethodCriteriaTypeOptions
            "
    ...
    <div
      class="bg-red-900/10 shadow-xl rounded-2xl border border-red-900/30 overflow-hidden"
    >
      <div class="px-6 py-5 border-b border-red-900/20 bg-red-900/20">
        <h2 class="text-lg font-bold text-red-400">
          {{ t("stores.settings_danger_zone") }}
        </h2>
      </div>
      <div class="px-6 py-6">
          ...
          <div class="sm:flex sm:items-start">
            ...
            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
              <h3
                ...
                id="modal-title"
              >
                {{ t("stores.settings_delete_confirm_title") }}
              </h3>
              <div class="mt-2">
                <p class="text-sm text-gray-400">
                  {{
                    t("stores.settings_delete_confirm_desc", {
                      name: store?.name || "",
                    })
                  }}
                </p>
                ...
                />
                <p v-if="deleteStoreError" class="mt-2 text-sm text-red-500">
                  {{ deleteStoreError }}
                </p>
              </div>
            </div>
...
const activeSettingsTab = ref<
  "settings" | "payment" | "rates" | "checkout" | "lightning" | "email"
>("settings");
const settingsTabs = [
...
const settingsForm = ref<Record<string, any>>({
  ...
  payment_method_criteria: [
    { payment_method: "BTC-LN", type: "GreaterThan", value: "" },
  ],
  timezone: "UTC",
  preferred_exchange: "",
});
...
const networkFeeModeOptions = [
  ...
  {
    label: "Only if customer makes more than one payment for the invoice",
    value: "MultiplePaymentsOnly",
  },
...
async function fetchSettings() {
  ...
  try {
    ...
    settingsForm.value.apply_brand_color_to_backend =
      !!d.apply_brand_color_to_backend;
    settingsForm.value.default_currency = d.default_currency ?? "EUR";
    settingsForm.value.additional_tracked_rates = arr(
      d.additional_tracked_rates,
    );
    // invoice_expiration: API returns seconds; UI shows minutes (15 min default)
    settingsForm.value.invoice_expiration = Math.round(
      (d.invoice_expiration ?? 900) / 60,
    );
    settingsForm.value.refund_bolt11_expiration =
      d.refund_bolt11_expiration ?? 30;
    // API returns seconds; UI shows minutes (BTCPay default 300s = 5min)
    settingsForm.value.display_expiration_timer = Math.round(
      (d.display_expiration_timer ?? 300) / 60,
    );
    // monitoring_expiration: API returns seconds; UI shows minutes (1440 min = 24h default)
    settingsForm.value.monitoring_expiration = Math.round(
      (d.monitoring_expiration ?? 86400) / 60,
    );
    settingsForm.value.speed_policy = d.speed_policy ?? "HighSpeed";
    settingsForm.value.lightning_description_template =
      d.lightning_description_template ?? "";
    ...
    settingsForm.value.lightning_private_route_hints =
      !!d.lightning_private_route_hints;
    settingsForm.value.on_chain_with_ln_invoice_fallback =
      !!d.on_chain_with_ln_invoice_fallback;
    settingsForm.value.lnurl_enabled = d.lnurl_enabled !== false;
    settingsForm.value.lnurl_classic_mode = d.lnurl_classic_mode !== false;
    settingsForm.value.lnurl_allow_payee_comment =
      d.lnurl_allow_payee_comment !== false;
    ...
    settingsForm.value.recommended_fee_block_target =
      d.recommended_fee_block_target ?? 1;
    ...
    settingsForm.value.show_pay_in_wallet_button =
      d.show_pay_in_wallet_button !== false;
    ...
    settingsForm.value.default_payment_method =
      d.default_payment_method ?? "BTC-LN";
    ...
    if (Array.isArray(rawCriteria) && rawCriteria.length > 0) {
      ...
      settingsForm.value.payment_method_criteria = [
        { payment_method: "BTC-LN", type: "GreaterThan", value: "" },
      ];
    }
...
async function handleSettingsSubmit() {
  ...
  try {
    ...
    if (typeof payload.display_expiration_timer === "number") {
      payload.display_expiration_timer = Math.round(
        payload.display_expiration_timer * 60,
      );
    }
    ...
    if (typeof payload.monitoring_expiration === "number") {
      payload.monitoring_expiration = Math.round(
        payload.monitoring_expiration * 60,
      );
    }
    ...
    if (!canEditCheckoutOptions.value) {
      const checkoutFields = [
        ...
        "receipt",
        "support_url",
        "display_expiration_timer",
        "redirect_automatically",
        "pay_join_enabled",
        "auto_detect_language",
        "show_pay_in_wallet_button",
        "show_store_header",
        "celebrate_payment",
        "play_sound_on_payment",
