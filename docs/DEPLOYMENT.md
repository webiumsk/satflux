...
Caddy mounts the whole [`docker/caddy/`](../docker/caddy/) directory (including `Caddyfile.relay` generated by `deploy.sh`). If the site returns **501/521** after a relay deploy, check `docker logs satflux_caddy_standalone` and restart Caddy after fixing the mount.
