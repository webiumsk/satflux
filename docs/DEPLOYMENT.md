...
| `satflux_evolu_relay_standalone` | Evolu relay (E2EE invoicing sync) |

## Evolu relay (local-first invoicing)

When `VITE_INVOICING_LOCAL_FIRST=true`, browsers sync invoicing data via WebSocket to Evolu relay(s). The standalone stack includes `evoluhq/relay:latest` (internal port 4000) with persistent volume `evolu_relay_data`.

Production setup (see [INVOICING_LOCAL_FIRST_ROLLOUT.md](INVOICING_LOCAL_FIRST_ROLLOUT.md)):

1. DNS **A** record: `relay.satflux.io` → VPS IP.
2. In `.env.standalone`: `RELAY_SITE_ADDRESS=relay.satflux.io`, `VITE_EVOLU_RELAY_URL=wss://relay.satflux.io`, `VITE_EVOLU_RELAY_URL_SECONDARY=wss://free.evoluhq.com`.
3. `./deploy.sh` (generates Caddy TLS site + rebuilds frontend).

Relay backup: `./backup.sh` archives `satflux_standalone_evolu_relay_data` when `BACKUP_EVOLU_RELAY=true`.
