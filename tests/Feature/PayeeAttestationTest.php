...
class PayeeAttestationTest extends TestCase
{
    ...
    #[Test]
    public function a_settled_lightning_payment_is_attested_when_its_destination_arrives_late(): void
    {
        Notification::fake();
        $this->fakeBtcPay();
        [$user, $store, $connection] = $this->connectedStore();
        ...
        // Some BTCPay payloads can report the payment as Settled before the
        // BOLT11 destination is populated. The first sync cannot judge it.
        $this->paymentStatus = 'Settled';
        $this->includeMethodDestination = false;
        $this->includePaymentDestination = false;
        $this->paidInvoices = [Bolt11Test::OTHER_INVOICE];
        ...
        // The row status stays Settled, but the now-usable destination must be
        // enough to run attestation for the same payment id.
        $this->includePaymentDestination = true;
        app(SettlementLedgerService::class)->syncInvoice($store, 'paid-1', forgetCache: true);
