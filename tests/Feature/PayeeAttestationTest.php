...
class PayeeAttestationTest extends TestCase
{
    ...
    #[Test]
    public function a_settled_lightning_payment_is_attested_when_the_destination_arrives_late(): void
    {
        Notification::fake();
        $this->fakeBtcPay();
        [$user, $store, $connection] = $this->connectedStore();
        ...
        // Some BTCPay webhook reads expose the payment as settled before the
        // BOLT11 destination is populated. That first sync cannot judge payee.
        $this->includeMethodDestination = false;
        $this->includePaymentDestination = false;
        $this->paidInvoices = [Bolt11Test::OTHER_INVOICE];
        app(SettlementLedgerService::class)->syncInvoice($store, 'paid-1');
        ...
        // When the destination appears later but the status stays Settled,
        // attestation must still run for the existing ledger row.
